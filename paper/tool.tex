...
\emph{type recovery} requires special techniques, a survey up until 2015 can be
...
Indeed, type recovery algorithms need both data and control flow to build type constraints,
...
The compiler is implemented in Rust, its source code and a short user guide are given at (cite).
Our effort is to attack the core of type recovery algorithms,
...
representations in the low-level machine code. Beside custom tricks, \emph{uCc} uses extensively
...
tested functions and their types in source code, table~\ref{flattening} shows results of using
decompilers to decompile binaries generated by \emph{uCc}. The test suite is also available at
the repository.

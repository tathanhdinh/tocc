...
Just to quote from (Automating Mimicry Attacks Using Static Binary Analysis)
\begin{quote}
...tracking function calls together with the corresponding parameters...
\end{quote}
% \quote{...tracking function calls together with the corresponding parameters...}

Thus the knowledge about types
...
low-level object may have different high-level types, this either comes from type erasure in
compilation (high-level objects with distinguished types are compiled into a
single low-level object, or casting used in the source code.
% (i.e. types assigned by the high-level type system).

One of the first discovered instances is pointer arithmetic, the expression $p + c$
...
of Van Emmerik (as with TIE and Retypd). Another instance is symbolic constants, e.g. after being reset
...
(of value \texttt{NULL}); the pattern is captured by Retypd only. A more complex example is shown in~\Cref{lst:divisionasm} (its source code

...
of our knowledge there is few effort in hiding type information. The most related research that we
can find is the work of Drape~\cite{drape_generalising_2007,drape_obfuscation_2004,drape_obfuscation_2006}
about abstract data type obfuscation, but he focused on complex data types (array, list, tree) using an
existing technique of Collberg~\cite{collberg_taxonomy_1997}: array splitting. The structure of such a
type actually makes more room for obfuscating techniques, and we consider them heavyweight since they
always insert much code size and runtime overhead.

\subsubsection*{Contribution}
This paper presents a method for hiding type information, the principal idea is based on the fact that the compiler does not need to preserve all information
...
Our method is very lightweight, type obfuscation is obtained with only a small code size and runtime
overhead; more sophisticated obfuscation can be added as an option. Though we only
present how it work on primitive types and function signatures but the idea can be applied on complex types.

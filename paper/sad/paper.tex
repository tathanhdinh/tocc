...
in~\Cref{lst:divisionasm}, its source code is in~\Cref{lst:divisionc}. For reasoning it would 
be better to look at the Cranelift IR  in~\Cref{lst:divisionclift} since the current type recovery methods work always 
on some IR. As we can see $\mathtt{eax}$ (or $\mathtt{edi}$) behaves as it is of type
...
9  ret
\end{lstlisting}
\begin{lstlisting}[style={cranelift},caption={Signed and unsigned division (source)},label={lst:divisionclift}]

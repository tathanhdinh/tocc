...
\texttt{replaceFunction}. For static analysis, the exploit generation for mimicry attacks 
given in~\cite{kruegel_automating_2005} requires a capability of
\begin{quote}
...
\texttt{\small const}, \texttt{\small volatile}), and in general any
...
different types (see~\Cref{subsec:polymorphism}). Let's consider an assignment
\begin{equation*}
  p' = p + n
\end{equation*}
...
\noindent
The later work always lifts machine code to an IR before type analysis. Lee et al. in
...
instead they focus on correct disassembling. That allows to 
...
To get the idea, let us consider the small piece of assembly code in~\Cref{lst:derefasm} (the original source code 
...
then the decompilation result (\Cref{lst:derefdec}). As we can see, RetDec does not bother to detect whether $p$ is a pointer to $\mathtt{int}$, 
...
Though only giving poor results about types, this approach has advantages: the decompilation is always possible after 
disassembling, the type system contains just basic types (being mapped from IR types) and casts.
...
(of value \texttt{NULL}); the instance is captured by Retypd only. 
...
\subsubsection*{Union}
A more complex instance is \emph{incompatible types}, an example is given in~\Cref{lst:divisionasm} 
(see~\Cref{lst:divisionc} for the source code). To avoid unnecessary details when using the assembly 
code for type analysis, it would be better to look at the cranelift IR version given in~\Cref{lst:divisionclift} 
...
Type constraints for $x$ and $y$ can be derived in a traditional way, for example with $x$
\begin{equation*}
...
\end{equation*}
TIE interprets $\vdash \tau_{x} <\colon \mathtt{i32} \sqcap \mathtt{u32}$ as $x \in \left[0, 2^{31}-1\right]$, 
which is not a correct constraint; while Retypd uses \emph{union} for the meet 
$\mathtt{i32} \sqcap \mathtt{u32}$, which is a reasonable result. Actually, the technique of using 
...
\subsection{Notion}\label{subsec:typeflatteningnotion}
\noindent
...
function pointer types. It may worth to note that $\mathtt{union}$ and $\mathtt{size}$ are not
defined for function types.
...
\end{figure}
The subtype relation between function types are defined in the traditional way, e.g.
\begin{equation*}

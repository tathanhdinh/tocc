...
the generated machine code (type erasure), then type recovering requires special techniques. Though a broad survey for research
...
instead of binding it early to some fixed type. He proposed using \emph{subtyping lattice} to express the
...
\begin{figure}[h]
  ...
  \end{tikzcd}
  \caption{Subtype lattice}
\end{figure}
$p'$ will not be early bound as $\mathtt{ptr}(S)$, instead $\vdash p' \colon \mathtt{void*}$
(adding integer to pointer does not result in pointer of the same type). The precise type
is only assigned later, when met with another type, derived from another use:
\begin{equation*}
  *{p'} + m
\end{equation*}
where $\vdash m \colon \mathtt{int}$, so $\vdash p' \colon \mathtt{ptr}(\mathtt{int})$.

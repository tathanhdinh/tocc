...
\begin{itemize}
  ...
  We introduce in \emph{uCc} a method to generate pointer aliases which would be hard to reverse thanks
  to MBA~\cite{zhou_information_2007}.
...
\noindent
In this section, we present our proposal for type flattening obfuscation and techniques used to obtain that notion. We
...
\noindent
The type recovery approaches proposed several techniques to deal with the problem of low-level polymorphism,
but a common point
...
context, under AMD SystemV ABI, it is $\mathtt{size64}$.
...
\end{lstlisting}
Any type recovery method can find the return type of \texttt{foo}. For example,
...
In~\cref{lst:obfmov}, we insert an alias $q$ for $p$ to hide the data-flow.
TIE derives $\vdash y \colon \mathtt{size32}$, while Retypd is not better
...
Our compiler \emph{uCc} disrupts the data-flow by: before an use (which needs to be protected) of a
variable, \emph{uCc} creates a stack slot to store it, generates an alias for the stack slot,
then loads the variable from the alias.

\subsection{Function signature masquerading}
\noindent
While types of local variables can be obfuscated by hiding the data-flow, obfuscating
...
\subsubsection*{Inner trampoline}
In the function, \emph{uCc} creates a trampoline as the pseudo-entry basic block, this trampoline uses
the masqueraded signature is to retrieve the parameters but then convert them to the real ones using
...
\subsubsection*{Semantics gap}
The techniques used for function signature masquerading have actually exploited a \emph{semantics gap}
...
no matter what its type is. Basically, the signature rewriting and inner trampoline have exploited the
low-level polymorphism to wrap a type by its supertype.

\subsection{Other techniques}
\noindent
\emph{uCc} uses also some supplemental techniques to augment its resistance from type analysis algorithms
and deobfuscation efforts in general.

\subsubsection*{Split and merge}
To bypass the type constraints generated from
$\mathtt{load}$/$\mathtt{store}$ operations in TIE, as well as
...
\emph{uCc} is a probabilistic compiler, the generation of pointer aliases is not the same over generated
binary codes: the coefficients and even the degree of the invertible polynomial are generated randomly for
each run of \emph{uCC}. The number of split parts in a value loading is randomized, as well as the size of each part.

\section{Implementation}
\noindent
In implementing \emph{uCc}, beside the principal goal is type flattening obfuscation, we try to avoid any
other obfuscation tricks that may bring unwanted effects on evaluating the obfuscation. Concretely,
we want that the functions in binaries generated by \emph{uCc} would be perfectly detectable,
disassemblable and decompilable (but with flattened types). For these goals, we have to skip almost all
popular obfuscation techniques~\cite{banescu_tutorial_2018,collberg_surreptitious_2009}.

\subsection{Pointer alias generation}
\noindent
...
generation of pointer aliases become straightforward. Basically, we generate randomly an invertible
polynomial $P$ over $\mathbb{Z}/2^n$ where $n$ is the machine word bit size ($n = 64$), and its
inverse $Q = P^{-1}$ (see~\cref{fig:invertiblepolynomial} for an example). Then the aliases of
some value $v$ is $Q(P(v))$, the calculation of $Q(P(v))$ is generated into the binary code.
...
\begin{figure}
  ...
  \end{align*}
  \caption{Invertible polynomial and its inverse}

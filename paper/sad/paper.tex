...
\begin{itemize}
  ...
  We introduce in \emph{uCc} a simple method to generate pointer aliases which would be hard to reverse thanks
...
From now on, unless otherwise stated, the target language is C, the type system is C's then.
...
variable, \emph{uCc} creates a stack slot to store it, next generates an alias for the stack slot,
finally loads the variable from the alias.
...
which is the capability of evaluating the single effect of type flattening. Concretely,
...
other obfuscation tricks that may bring unwanted effects, almost all
popular obfuscation techniques~\cite{banescu_tutorial_2018,collberg_surreptitious_2009} cannot be
applied.

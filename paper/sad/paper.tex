...
$\mathtt{i32} \sqcap \mathtt{u32}$, which is a reasonable result. Actually, using union to combine 
incompatible types is similar to the type casting approach of untyped decompilers discussed above.
...
\end{lstlisting}
The illustrative assembly code and cranelift IR show also that in low-level, data copy does not
always result in \emph{type copy}: two identical values may have different types. For example, 
...
of $\mathcal{P}$ is either of form $\mathtt{i}N$ (for signed types) or $\mathtt{u}N$ (for unsigned types):
\begin{equation*}
...
\end{align*}
The constructor $\mathtt{ptr}$ is extended to cover union types, naturally the size of a pointer to 
union is $8$.
...
For typing functions of multiple parameters, there is an implicit product type constructor $()$,
e.g. if $f \colon \tau_{x} \to (\tau_{y}, \tau_{z})$ then $f$ is of type $\to(\tau_{x}, (\tau_{y}, \tau_{z}))$.
For simplification, we limit that $f$ is only defined over primitives, pointers of primitives, and 
unions of primitives. It may worth to note that $\mathtt{union}$ is not defined for functional types.

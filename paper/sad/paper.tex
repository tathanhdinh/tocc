...
$\mathtt{i32} \sqcap \mathtt{u32}$, which is a reasonable result.
...
\noindent
In this section, we present our proposal for the notion of type flattening obfuscation and techniques 
used in \emph{uCc} to obtain that notion. We focus only on scalar types (pointers, integers, 
but not floats), and function signatures where parameters are scalar types; supporting aggregate types
...
\noindent
Here we define a simple type system with subtyping 

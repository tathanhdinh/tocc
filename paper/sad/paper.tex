...
\hypersetup{
  colorlinks = true,
  urlcolor = black,
  pdftitle = "A Lightweight Obfuscation for Function Signature Flattening",
...
\lstloadlanguages{C, [x86masm]Assembler, LLVM}
% \lstloadlanguages{C}
% \lstset{
...
\lstdefinestyle{cranelift}{
  language=LLVM,
  morekeywords={iadd, return}
}
\usepackage{cleveref}
...
type makes more room for obfuscation, and his approach is orthogonal with ours (see~\Cref{sec:relatedwork}).
...
% We find that Mixed Boolean Arithmetic (MBA) expressions~\cite{eyrolles_obfuscation_2017,zhou_information_2007} are a good match for the goal.
...
\begin{itemize}
  \item We give a critical analysis (\Cref{subsec:polymorphism}) about how current binary type recovery methods deal with
  ...
  \item We introduce the notion of \emph{type flattening} obfuscation (\Cref{subsec:typeflatteningnotion}), it aims at protecting a high-level
  ...
  % We introduce in \emph{uCc} a simple method to generate pointer aliases which
...
different types (see~\Cref{subsec:polymorphism}). Let us consider an assignment
\begin{equation*}
  p' = p + n
\end{equation*}
...
low-level polymorphism problem (see~\Cref{subsec:polymorphism}).
% (i.e. an expression can behaves as different types)
...
\begin{figure}[h]
  \centering
  \begin{tikzcd}
    \mathtt{.load} \arrow[d, no head] \\
    \mathtt{ptr}(\mathtt{int})
  \end{tikzcd}
  \caption{Tree for derived type when reading a pointer of type $\mathtt{int}$}
  \label{fig:derivedtype}
\end{figure}
% Noonan et al. handled these problems in Retypd~\cite{noonan_polymorphic_2016} by first using
Retypd uses subtyping in almost all derived type constraints.
% The effect of data moving $x = y$ will be represented by
% $\vdash \tau_{y} <\colon \tau_{x}$. More importantly, they proposed a \emph{type capability} model: each type variable is attached with several labels representing it capabilities.
...
% Retypd uses subtype lattices, as in TIE and Van Emmerik's.
% and type analysis is proceeded on an IR, as in TIE.
...
% Only Van Emmerik gives an open source implementation of his method in the Boomerang decompiler,
% Lee et al and Noonan et al. do not. Published recently, Ghidra~\cite{noauthor_ghidra_nodate} is an
% open source decompiler with type recovery.
% Snowman~\cite{noauthor_snowman_nodate} and
% RetDec~\cite{kroustek_retdec_2017} seem do not focus much on type analysis.
% There are also commercial tools, most notably Hex-Rays~\cite{noauthor_hex-rays_nodate} and
% JEB~\cite{noauthor_jeb_nodate}, their methods are not published.
...
% A notable example is the retargetable decompiler 
% RetDec~\cite{kroustek_retdec_2017} which raises binary codes to LLVM~\cite{lattner_llvm_2004}.
% To get the idea, 
% Let us consider an example of RetDec~\cite{kroustek_retdec_2017}, the assembly code in~\Cref{lst:derefasm} (the original source code is 
% in~\Cref{lst:derefc}) will be raised to LLVM (\Cref{lst:derefllvm}), then decompiled to C (\Cref{lst:derefdec}).
% assembly code in~\Cref{lst:derefasm} (the original source code 
% is given in~\Cref{lst:derefc}).
Given the following example:
\begin{lstlisting}[language={[x86masm]Assembler},caption={Pointer increment then dereference},label={lst:derefasm}]
1  mov eax, [rdi+0x4]
2  ret
\end{lstlisting}
% RetDec raises the assembly code into LLVM (\Cref{lst:derefllvm}), 
% then decompiles to C (\Cref{lst:derefdec}). As we can see, RetDec does not bother to detect whether $p$ is a pointer to $\mathtt{int}$, 
...
\end{lstlisting}
The decompiler does not bother if $p$ is a pointer to $\mathtt{int}$ or not (see the original source code in~\Cref{lst:derefc}), 
just uses the ABI 
...
% this can lead to incorrect results because type casting is not always safe. 

% The recovered types are poor, they are mostly primitive types being mapped directly from IR types 
% with casts. Even worse, this can lead to incorrect results because type casting is not always safe. 
% For the example above, there may exist a pointer misalignment: $\mathtt{p} + 4$ may
% not be a valid address for a $\mathtt{int32\_t}$ pointer.
% , and a $\mathtt{int32\_t}$ pointer cannot always
% cast to a $\mathtt{int64\_t}$ pointer.
% This approach has poor result about types, high-level types are primitive types being mapped directly from IR types 
% with casts.
...
% (i.e. types assigned by the high-level type system).
...
% \begin{equation*}
%   \mathtt{xor} \quad \mathtt{rax}, \medspace \mathtt{rax}
% \end{equation*}
...
the type system.
  % the instance is recognized by Retypd only. 
...
A more complex phenomenon but ubiquitous in low-level code is \emph{incompatible types}. An example is in~\Cref{lst:divisionasm} 
(see~\Cref{lst:divisionc} for the source code); to avoid unnecessary details of using the assembly 
code for type analysis, it would be better to look at the cranelift IR~\cite{noauthor_cranelift_nodate} version given in~\Cref{lst:divisionclift} 
(since the binary type recovery eventually works on some IR).
% As we can see $\mathtt{eax}$ (or $\mathtt{edi}$) behaves as it is of type signed and unsigned integer.
TIE has troubles in deriving type constraints for $x$ and $y$, 
% as with the case of symbolic constants, 
e.g. with $x$
\begin{equation*}
...
% but it does not discuss any further 
% for the meet $\mathtt{i32} \sqcap \mathtt{u32}$, which is a reasonable result. Actually, the technique of using 
% union to combine incompatible types is \emph{type punning}, another approach is \emph{type casting} 
% which is used by untyped decompilers discussed above.
% This situation is captured in Retypd by a seemingly controversial rule:
% $\alpha = \beta$ derives $\tau_{\beta} <\colon \tau_{\alpha}$ 
% (while with TIE $\tau_{\alpha} = \tau_{\beta}$) which relaxes the type of $x$ in data copy 
% operations (note that each $x$ in $\mathtt{udiv}$ and $\mathtt{sdiv}$ is actually a copy of the original 
% argument $x$)
% \begin{align*}
%   \tau_{x_{s}} <\colon \mathtt{i32} &\quad \tau_{x_{s}} <\colon \tau_{x} \\
%   \tau_{x_{u}} <\colon \mathtt{u32} &\quad \tau_{x_{u}} <\colon \tau_{x}
% \end{align*}
%  at line 1 and line 6. 
% Retypd captures also
% another kind of low-level polymorphism called \emph{physical subtyping}~\cite{siff_coping_1999},
% which is layout-safe casting between structs.
...
the register \texttt{\small eax} at line 1 and line 6 in~\Cref{lst:divisionasm} are two identical copies of 
...
% In summary, incompatible types are combined using union. This technique is actually similar to the type casting 
% approach of untyped decompilers discussed above.

% In general, whenever there are conflicts between types, Retypd can use union to combine them. This 
% technique is similar to the type casting approach used extensively by untyped decompilers 
% discussed above.

% But the problem is that there are also many instances which are completely computational
% (i.e. beyond which a set of limited type propagation rules can reach), they
% are generated either \raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {1}}} by the compiler after
% the type checking, thus independent with the high-level type system (type erasure);
% or \raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {2}}} by casting, thus bypass the type system.
% In summary, there exist always instances that no high-level type system, among the ones discussed
% above, can capture. To end the discussion about low-level polymorphism, let us look at the following expression
% \begin{equation*}
%   (\mathtt{void} *)((\mathtt{uintptr\_t})p + \mathtt{PAGESIZE} - 1 \& \sim(\mathtt{PAGESIZE} - 1))
% \end{equation*}
% which is used to roundup a pointer to a page boundary~\cite{noauthor_mprotect_nodate} before
% passing to $\mathtt{mprotect}$.
% The pointer $p$ is cast to use as an integer, but the value of the expression
% is cast back as a pointer. All previously discussed binary type recovery methods have difficulties in dealing with this
% expression.
...
\end{equation*}
% Naturally, $\mathtt{union}$ is idempotent, i.e. $\mathtt{union}(\tau_{x}, \tau_{x}) = \tau_{x}$.
...
\end{align*}
% The constructor $\mathtt{ptr}$ is extended to cover union types, naturally the size of a union pointer is $8$.
...
% i.e. if some function $f \colon \tau_{x} \to \tau_{y}$ then $f$ is of type $\to(\tau_{x}, \tau_{y})$.
For functions of multiple parameters, there is an implicit product type constructor $()$,
% e.g. if $f \colon \tau_{x} \to (\tau_{y}, \tau_{z})$ then $f$ is of type 
whose meaning is direct from the context,
e.g. $\tau_{x} \to (\tau_{y}, \tau_{z})$.
% for function of return type $\tau_{x}$ and having two parameters
% of types $\tau_{y}$ and $\tau_{z}$.
...
denote it $\tau$), then $\tau_{x} <\colon \tau$ and $\tau_{y} <\colon \tau$ (see also~\Cref{fig:unionsupertype}).
\begin{figure}[h]
  \centering
  \begin{tikzcd}
    & \mathtt{union}(\tau_{x}, \tau_{y}) \arrow{dl}[sloped,above]{<\colon} \arrow{dr}[sloped, above]{\colon>} & \\
    \tau_{x} & & \tau_{y}
  \end{tikzcd}
...
% \begin{align*}
%   \tau_{x} <\colon \mathtt{union}(\tau_{x}, \tau_{y}) \\
%   \tau_{y} <\colon \mathtt{union}(\tau_{x}, \tau_{y})
% \end{align*}



% $\mathtt{union}$ is symmetric, i.e. $\mathtt{union}(\tau_{x}, \tau_{y}) = \mathtt{union}(\tau_{y}, \tau_{x})$

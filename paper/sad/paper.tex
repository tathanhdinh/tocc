...
To handle these problems, Noonan et al.~\cite{noonan_polymorphic_2016} used subtyping relations
in almost all deduction rules. The effect of data moving $x = y$ will be represented by
$\vdash T(y) <\colon T(x)$. More importantly, they propose a
\emph{type capability} model where each type is attached with several labels representing it capabilities.
...
the type of the dereferenced value is a subtype of type of $x$. The labels on $p$ allows to

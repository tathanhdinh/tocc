...
we will presented in the section, the low-level polymorphism is a very specific problem that binary type
...
\noindent
In this section, we present proposal for type flattening obfuscation and techniques used for that notion. We
...
\subsubsection*{Goal}
Our goal in type flattening obfuscation is to make any referred type become $\top$. In our specific
context, under AMD SystemV ABI, it is $\mathtt{size64}$.

\subsection{Hiding data-flow}
\noindent
...
A direct trick for type flattening is to hide the data-flow in the program, that is done by
introducing \emph{spurious pointer aliases}~\cite{collberg_surreptitious_2009}. Let's look at the
piece of code in~\cref{lst:directmov}, for illustration purpose we have used a pseudo-C syntax but only
add type annotation on the variables whose types are supposed to be already known.
...
In~\cref{lst:obfmov}, we hide the data-flow by adding a spurious alias $q$ for $p$.
TIE would derive $\vdash y \colon \mathtt{size32}$, while Retypd is not better
$\vdash \mathtt{size32} <\colon y$ where $\vdash y <\colon \mathtt{size32}$.
\begin{lstlisting}[style={c},caption={Data movement with spurious alias},label={lst:obfmov}]
foo(signed32 x) {
  p = &x;
  q = a spurious alias of p;
  y = *q;
  return y;
}
\end{lstlisting}

\subsection{Hiding function signatures}
\noindent
We have seen that types of local variables can be hidden by corrupting the data-flow, but hiding
types of function parameters require different tricks. For inter-procedural type analysis,
the data-flow corruption still works because the type of passed arguments are already hidden: they
are local variables of the calling function. But for intra-procedural type analysis, function
parameters are one of type information sources; with the previous trick, we can still hide
...
In binaries compiled by a normal compiler, the size of a function parameter can be detected
by the size of the register used inside the function. For example, if the first argument is of type
$\mathtt{int}$ (i.e. $\mathtt{signed32}$) then the register \texttt{edi} is used, not \texttt{rdi}.
In our compiler \emph{uCc}, the function parameter size is hidden by following tricks.

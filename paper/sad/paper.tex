...
\texttt{replaceFunction}. Or the binary exploit generation for mimicry attacks 
...
\subsubsection*{Captured instances} One of the first recognized forms is \emph{pointer arithmetic}; 
...
in~\Cref{lst:divisionasm}, its source code is in~\Cref{lst:divisionc}. Since the binary type recovery 
eventually works on some IR, to avoid unnecessary details when using the assembly code for type 
constraint propagation reasoning, it would be better 
to look at the cranelift IR version given in~\Cref{lst:divisionclift}. 
% As we can see $\mathtt{eax}$ (or $\mathtt{edi}$) behaves as it is of type signed and unsigned integer.
\begin{lstlisting}[style={cranelift},caption={Signed and unsigned division (cranelift)},label={lst:divisionclift}]
...
\end{lstlisting}
It is captured in Retypd by a seemingly controversial rule:

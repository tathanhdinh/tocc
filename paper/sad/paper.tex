...
\begin{itemize}
  \item We give a critical analysis (see~\Cref{subsec:polymorphism}) about how current binary type recovery methods deal with
  ...
  \item We introduce the notion of \emph{type flattening} (see~\Cref{subsec:typeflatteningnotion}), it aims at protecting a high-level
...
\noindent
The later work always lifts machine code to an IR before type analysis.
Lee et al. in TIE~\cite{lee_tie_2011} had a similar idea of using subtype lattice,
their type system are more detailed and support more cases thanks to the IR
(e.g. struct/function types, constraints for call/jump) but basically
similar to Van Emmerik's. For example, the previously discussed assignment
\begin{equation*}
  p' = p + n
\end{equation*}
...
the assignment) then this constraint is equivalent with $\vdash p' \colon \mathtt{void*}$.
% The notable
...
\emph{low-level polymorphism} problem (see~\Cref{subsec:polymorphism}).
...
whose the first field is also of type pointer to this struct and $p$ itself. That way, Retypd
...
\emph{low-level polymorphism} in their type systems. This kind of polymorphism means an low-level expression
behaves as it has different high-level types (i.e. types that a high-level type system assigns to it).
One of the instances is pointer arithmetic, the expression $p + c$
where $p$ is a pointer and $c$ is some constant, may be of type $\tau_{p}$ (same type with the pointer
$p$) or just an unknown pointer type $\tau_{\alpha}$. This pattern is captured by the type system
of Van Emmerik (as with TIE and Retypd). Another instance is zero value,
after the operation
\begin{equation*}
  \mathtt{xor} \quad \mathtt{rax}, \medspace \mathtt{rax}
\end{equation*}
the register $\mathtt{rax}$ can be used both as an integer and as a \texttt{NULL} pointer; the pattern
is captured by Retypd only. A more complex instance is shown in~\Cref{lst:divisionasm} (its source code
is in~\Cref{lst:divisionc}), as we can see $\mathtt{eax}$ (or $\mathtt{edi}$) behaves as it is of type
...
9  ret
\end{lstlisting}
Retypd captures this instance by a seemingly unintuitive rule:
$x = y$ derives $\tau_{y} <\colon \tau_{x}$ (while with TIE $\tau_{y} = \tau_{x}$) which relaxes
the types of $\mathtt{eax}$ in the data copy operations at line 1 and line 6. Retypd captures also
another class of low-level polymorphism instances called \emph{physical subtyping}~\cite{siff_coping_1999},
...
\noindent
In this section, we present our proposal for type flattening notion and techniques used in

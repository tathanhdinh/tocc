...
\begin{itemize}
  \item We introduce the notion of \emph{type flattening} (in~\Cref{subsec:typeflatteningnotion}), it aims at protecting a high-level
...
\emph{low-level polymorphism} in their type systems. Naturally, that means an low-level expression
...
of Van Emmerik (and both TIE and Retypd). Another instance is zero value,
after the operation
\begin{equation*}
...
9  ret
\end{lstlisting}
Retypd captures this instance partially using a seemingly unintuitive rule
$x = y$ derives $\tau_{y} <\colon \tau_{x}$ (while with TIE $\tau_{y} = \tau_{x}$) which relaxes
the type of $\mathtt{eax}$ in the data movement operations at line 1 and line 6. Another class of
patterns called \emph{physical subtyping}, its treatment can be referenced in~\cite{siff_coping_1999}.
...
they are generated either by the compiler after the type checking, thus independent with the high-level type
system (type erasure); or by casting, thus bypass the type system. In summary, there exist always instances that no high-level type system, with fixed set of
type propagation rules as discussed above, can capture. To end the discussion, the following expression
is used to roundup a pointer to a page boundary~\cite{noauthor_mprotect_nodate} before passing
to $\mathtt{mprotect}$
\begin{equation*}
  (\mathtt{void} *)((\mathtt{uintptr\_t})p + \mathtt{PAGESIZE} - 1 \& ~(\mathtt{PAGESIZE} - 1))
\end{equation*}
As we can see, the pointer $p$ is used just as an integer, but the value of the expression is used
as a pointer.

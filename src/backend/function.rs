...
pub fn translate_function<'clif, 'tcx>(
	...
	let mut func_translator =
		FunctionTranslator::new(func_builder, module, func_id, return_ty, name_env, outer_type_env);
	...
	fn translate_binary_operator_expression(
		...
		match (lhs_val, rhs_val) {
			...
			(ConstantTy(lhs), ValueTy(rhs)) => match operator {
				Multiplication => ValueTy(self.imul_imm(rhs, lhs)),
				Division => {
					let rhs_ty = self.value_type(rhs);
					let lhs = self.iconst(rhs_ty, lhs);
					ValueTy(self.idiv(lhs, rhs))
				}
				Remainder => {
					let rhs_ty = self.value_type(rhs);
					let lhs = self.iconst(rhs_ty, lhs);
					ValueTy(self.srem(lhs, rhs))
				}
				Addition => ValueTy(self.iadd_imm(rhs, lhs)),
				Subtraction => {
					let rhs_ty = self.value_type(rhs);
					let lhs = self.iconst(rhs_ty, lhs);
					ValueTy(self.isub(lhs, rhs))
				}

				Less => ValueTy(self.icmp_imm(IntCC::SignedGreaterThan, rhs, lhs)),
				LessOrEqual => ValueTy(self.icmp_imm(IntCC::SignedGreaterThanOrEqual, rhs, lhs)),
				Greater => ValueTy(self.icmp_imm(IntCC::SignedLessThan, rhs, lhs)),
				GreaterOrEqual => ValueTy(self.icmp_imm(IntCC::SignedLessThanOrEqual, rhs, lhs)),
				Equal => ValueTy(self.icmp_imm(IntCC::Equal, rhs, lhs)),
					...
					checked_match!(operator, Indirect, {
						checked_match!(ty, PointerTy(pty), {
							checked_match!(pty.as_ref(), AggregateTy(aggre_ty), {
								...
								let ident_val = self.use_var(*ident);
								ValueTy(self.load(*ty, ident_val, offset as i32))
							})
						})
					})
				}
